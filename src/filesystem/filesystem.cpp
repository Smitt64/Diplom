...
char *FileSystem::fsGetFName(QString fileName)
{
    ...
    char *exst = FileSystem::fsGetExstension(fileName);

    for(int i = fileName.length() - 1; i > (fileName.length() - size - 1); i--)
    {
        name[ch] = fileName[i].toAscii();
        ch--;
    }

    if(size > 80)
    {
        int esize = strlen(exst);
        for(int i = 0; i < esize; i++)
        {
            name[80 - esize + i - 1] = exst[i];
        }
        name[80 - esize - 2] = '.';
        name[80 - esize - 3] = '~';
    }
